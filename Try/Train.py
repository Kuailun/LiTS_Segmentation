from Try.Datasets import LiTS_Data
from Try.Model.mModel import *
from torch.utils.data import DataLoader
from torch import optim
from Try import Loss, Utils as ut, mPath
import torch
    ...
    for epoch in range(epochs):
        ...
        for phase in ['train','val']:
            ...
            for index,sample in enumerate(dataset):
                ...
                print('{} Loss:{:.6f} Acc:{:.10f} Liver_Dice:{:.10f}'.format(index, subloss, subaccTotal,sub_dice_liver))
                pass
            if not (dataLength==0):
                epoch_loss=running_loss/dataLength
                epoch_acc=running_corrects/dataLength
            ...
            print('{} Loss:{:.6f} Acc:{:.10f} Liver_Dice:{:.10f}'.format(phase,epoch_loss,epoch_acc,epoch_dice_liver))
            ...
            if epoch%Output_per_epoch==0:
                ut.plot_img(img[0,0,:,:], mPath.DataPath_Log + "Input0-" + str(epoch) + ".jpg", "Input", classes)
                ut.plot_img(mask[0, :, :, :], mPath.DataPath_Log + "Mask0-" + str(epoch) + ".jpg", "Mask", classes)
                ut.plot_img(preds[0, :, :], mPath.DataPath_Log + "Output0-" + str(epoch) + ".jpg", "Output", classes)
            pass
        pass
    writer.close()
    pass
...
if __name__=='__main__':
    ...
    try:
        ...
        train_net_1(net, lr=learning_rate, epochs=Train_Epochs, batch_size=Train_Batch_Size,val_percent=Validation_Percent, save_cp=Save_CheckPoint, gpu=Use_GPU, classes=Output_Class)
