...
for i in range(0,70,1):
    name1='test-tumor-'+str(i)+'.nii'
    name2=str(i)+'-ori.nii'
    name3='test-segmentation-'+str(i)+'.nii'
    img1, img1_header = load(mPath.DataPath_Volume_Predict + name1)
    img2, img2_header = load(mPath.DataPath_Volume_Truth + name2)
    layer=img1.shape[2]
    ...
    nib.save(new_img, mPath.DataPath_Volume_Predict + name3)